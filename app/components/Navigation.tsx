...
export default function Navigation() {
  ...
  const logNavigation = async (destination: string, action: string = 'NAVIGATION') => {
    if (session?.user?.email) {
      try {
        await fetch('/api/activity/log', {
          ...
          body: JSON.stringify({
            ...
            action,
            category: 'NAVIGATION',
            details: { destination, currentPath: pathname }
          })
        })
      } catch (error) {
  ...
  const handleLinkClick = (destination: string) => {
    logNavigation(destination, 'PAGE_NAVIGATION')
  }

  const handleSignOut = () => {
    logNavigation('/auth/signin', 'USER_SIGNOUT')
    signOut({ callbackUrl: '/' })
  }
  ...
  // Only show organizer links for organizers and admins
  const organizerLinks = (session && (userRole === 'organizer' || userRole === 'admin')) ? [
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '4rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
            ...
            {/* Venue Owner Dropdown - Only for Organizers and Admins */}
            {session && organizerLinks.length > 0 && (
